...
try:
    ...
    print(f"Error: Missing MCP dependencies. Please install: pip install fastmcp mcp nest_asyncio")
...
class LangFlowConnectMCPServer:
    ...
    def setup_tools(self):
        """Register all MCP tools using decorators"""
    ...
    def _register_langflow_tools(self):
        """Register LangFlow integration tools"""
    ...
    # Workspace Operations Tools Implementation
    ...
    # Cost Tracking Tools Implementation
    ...
    # LangFlow Integration Tools Implementation
    async def connect_to_langflow_tool_impl(self, websocket_url: str, 
                                           auth_token: str) -> str:
        """Connect to LangFlow"""
        try:
            if not self.system_coordinator:
    ...
    # System Management Tools Implementation
    ...
    async def run(self):
        ...
        try:
            ...
            # FastMCP needs to run in its own event loop context
            # We'll use a different approach for now - just keep the server alive
            self.logger.info("MCP Server is ready and waiting for connections...")
            self.logger.info("Note: This is a development mode. For production, use the proper MCP client.")
